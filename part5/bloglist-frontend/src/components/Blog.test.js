...
test('renders blog', () => {
    ...
    render(<Blog blog={blog} handleLikeChange={emptyFunc} handleRemove={emptyFunc} loggedUser={{}} />)
...
test('shows blog details on button click', async () => {
    const { container } = render(<Blog blog={blog} handleLikeChange={emptyFunc} handleRemove={emptyFunc} loggedUser={{}} />)
...
test('event handler is called twice when button is clicked twice', async () => {
    ...
    const { container } = render(<Blog blog={blog} handleLikeChange={mockHandler} handleRemove={emptyFunc} loggedUser={{}} />)
    const user = userEvent.setup()
